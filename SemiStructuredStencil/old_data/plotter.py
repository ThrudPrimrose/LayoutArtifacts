...
for i, N in enumerate(N_values):
    ...
    ax.text(index[i] - bar_width / 2, median_no + 0.9, f"{median_no:.2f} ms", ha='center', va='bottom', fontsize=8, color='gray')

    # Calculate speedup and display it on top of the orange bar (if median_no > 0)
    if median_no > 0:
        speedup = median_no / median_yes
        ax.text(index[i] + bar_width / 2, median_yes + 0.09 if N != "258" else median_yes + 0.05, f"{speedup:.2f}x", ha='center', va='bottom', fontsize=8, color='black')
