...
unique_benchmarks = data.loc[data["framework"] == "mi300a dace_gpu", "benchmark"].unique()
...
filtered_data = filtered_data[~((filtered_data["framework"] == "a100 dace_gpu") |
                                (filtered_data["framework"].str.endswith("auto_tile")))]
print("F\n", filtered_data['framework'].unique())
data = filtered_data
print("D\n", data)
...
# Drop rows where column 'A' is in remove_list
data = data[~data['benchmark'].isin(remove_list)]
...
ticks = ax1.set_xticklabels([v.replace("dace_gpu", "").replace("mi300a", "MI300A").replace("mi250x", "MI250X").replace("numpy", "NumPy").replace("gh200","GH200") for v in hm_data.columns])
...
plt.tight_layout()
plt.savefig("heatmapx1.pdf", dpi=600)
plt.savefig("heatmapx1.png", dpi=600)
plt.show()