...
def _greedy_search_row(filtered_df3, lim=1):
    rows = []
    prev_row = None
...
def search2(df, initial_guess, title, label, color, ylim, current_row, num_tests=0):
    ...
    old_cur_row = None
    print(cur_tX, cur_tY, cur_bK, cur_tM, cur_tN)
    while out != set(["tM", "tN"]):
        filtered_df1 = df[
            (df["tX"] == cur_tX)
            & (df["tY"] == cur_tY)
        ...
        t = turn
        filtered_df2 = filtered_df2.sort_values(by=t, ascending=True)

        old_cur_row = next_cur_row
        next_cur_row, _nt, rows, fail = _greedy_search_row(filtered_df3=filtered_df2, lim=1)
        #print("AA", next_cur_row, _nt, rows, fail, turn)
        i = 0
        if fail:
            out.add(turn)
        #pass
        turn = "tN" if turn == "tM" else "tM"
        ...
        for r in rows:
            ...
            i += 1
        #print(out)
        #if fail and out == set(["tM", "tN"]):
        #    next_cur_row = old_cur_row
        #    break

        if fail:
            if old_cur_row is not None:
                __tY, __tX, __bK, __tM, __tN = old_cur_row[["tY", "tX", "bK", "tM", "tN"]].values
                cur_tX = __tX
                cur_tY = __tY
                cur_bK = __bK
                cur_tM = __tM
            ...
            __tY, __tX, __bK, __tM, __tN = next_cur_row[["tY", "tX", "bK", "tM", "tN"]].values
            cur_tX = __tX
            cur_tY = __tY
            cur_bK = __bK
            cur_tM = __tM
    ...
    next_cur_row, _nt, rows, fail  = _greedy_search_row(filtered_df3=filtered_df3, lim=500)
    checked_rows = pd.concat([checked_rows, *rows]).drop_duplicates().reset_index(drop=True)
    __tY, __tX, __bK, __tM, __tN = next_cur_row[["tY", "tX", "bK", "tM", "tN"]].values
    cur_tX = __tX
    cur_tY = __tY
...
for dev, ylim in devs:
    ...
    #  (outer_product_df, f"{dev}_Outer-Product"), (inner_product_no_rem_df, f"{dev}_Inner-Product CTO"),
    for _df, _name in [(inner_product_no_rem_df, f"{dev}_Inner-Product")]: #outer_product_df, inner_product_rem_df
        ...
        rr, _pre_tests, rows, fail = _greedy_search_row(filtered_df3=filtered_df0)
