...
__device__ __forceinline__ void __load7(float *shr, const float *glb, const int offsetY, const int offsetX, const int tid, const int glbX)
{
...
__device__ __forceinline__ void __storeToGlb7(float *glb, const float *shr, const int offsetY, const int offsetX, const int tid, const int glbX)
{
  constexpr int NumThreads = threadsY * threadsX;
...
constexpr inline int alignTo32(int a)
{
  return 32 * ((a + 31) / 32);
}
...
__global__ void _2d_block_tiling7(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C,
                                  const int M, const int N, const int K)
{
  ...
  constexpr int maxMemNeeded = alignTo32(StoreLength * ThreadsY * BN) > alignTo32(BM * BK) + alignTo32(BK * BN) ? alignTo32(StoreLength * ThreadsY * BN) : alignTo32(BM * BK) + alignTo32(BK * BN);
  __shared__ float shrMem[maxMemNeeded];
  float *As = &shrMem[0];
  float *Bs = &shrMem[alignTo32(BM * BK)];
...

  {
    ...
    {
      __load7<ThreadsY, ThreadsX, BM, BK>(As, A, globalRow, bkIdx, tid, K);
      __load7<ThreadsY, ThreadsX, BK, BN>(Bs, B, bkIdx, globalCol, tid, N);
...
#pragma unroll
      ...
      __syncthreads();
      __storeToGlb7<ThreadsY, ThreadsX, BN, StoreLength, TM>(C, Cs, globalRow + resIdxM, globalCol, tid, N);
        __syncthreads();
    }
  }
}
...
void _2d_block_tiling_launcher7(const float *__restrict__ A,
                                ...
                                const int M, const int N, const int K)
{
  ...
  _2d_block_tiling7<ThreadsY, ThreadsX, BK, TM, TN, StoreLength><<<grid, block>>>(A, B, C, M, N, K);
}