...
__device__ __forceinline__ void __load_cg(float *shr, const float *glb, const int offsetY, const int offsetX, const int tid, const int glbX)
{
...
__global__ void matmul_cg(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C,
                   ...
                   const int strideA, const int strideB, const int strideC)
{
  ...
  {
    __load_cg<ThreadsY, ThreadsX, BM, BK>(As, A, globalRow, bkIdx, tid, strideA);
    __load_cg<ThreadsY, ThreadsX, BK, BN>(Bs, B, bkIdx, globalCol, tid, strideB);
...
#pragma unroll
    for (int resIdxN = 0; resIdxN < TN; ++resIdxN)
    {
      C[(outputRow + resIdxM) * strideC + outputCol + resIdxN] = threadResults[resIdxM * TN + resIdxN];
    }
  }
}

template <int ThreadsY, int ThreadsX, int BK, int TM, int TN>
void matmul_cg_launcher(cudaStream_t stream, const float *__restrict__ A,
                 ...
                 const int strideA, const int strideB, const int strideC)
{
  ...
  matmul_cg<ThreadsY, ThreadsX, BK, TM, TN><<<grid, block, 0, stream>>>(A, B, C, M, N, K, strideA, strideB, strideC);
}