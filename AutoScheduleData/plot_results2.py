...
unique_benchmarks = data.loc[data["framework"] == "mi300a dace_gpu_auto_tile", "benchmark"].unique()
filtered_data = data[data["benchmark"].isin(unique_benchmarks)]
print("D\n", filtered_data)
print("F\n", filtered_data['framework'].unique())
filtered_data = filtered_data[~((filtered_data["framework"] == "a100 dace_gpu") |
                                (filtered_data["framework"] == "gh200 dace_gpu") |
                                (filtered_data["framework"] == "mi250x dace_gpu_auto_tile"))]
print("F\n", filtered_data['framework'].unique())
data = filtered_data
print("D\n", data)
...
ticks = ax1.set_xticks([v for v  in np.arange(len(hm_data.columns))])
ticks = ax1.set_yticks(np.arange(len(best_wide['benchmark'])))
# ... and label them with the respective list entries
ticks = ax1.set_xticklabels([v.replace("auto_tile", "Auto-Tile").replace("_", " ").replace("dace gpu", "").replace(" ", " ").replace("mi300a", "MI300A").replace("mi250x", "MI250X").replace("numpy", "NumPy").replace("gh200","GH200") for v in hm_data.columns],
                            ha="center", )
...
plt.tight_layout()
plt.savefig("heatmap2.pdf", dpi=600)
